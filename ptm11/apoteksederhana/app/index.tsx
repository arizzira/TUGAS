...
export default function Index() {
  ...
  const [gambarLocal, setGambarLocal] = useState("");
...
async function uploadCloudinary(uri: string) {
...
async function simpanObat() {
  if (
    ...
    Alert.alert(
      "Peringatan",
      "Semua data dan gambar wajib diisi"
    );
    return;
  }
  ...
  try {
    ...
    if (gambarLocal !== "") {
      urlGambar = await uploadCloudinary(gambarLocal);
    }
...
function pilihEdit(item: Obat) {
  ...
  setGambarLocal(item.gambar_url);
}
